  ...
  async executeCommand(args, msg, params) {
    ...
    if (args.length == 1) {
      ...
      messageHandler.sendRichTextDefault({
        msg: msg,
        title: replaceArgs(language.commands.register.labels.removeTitle, [
          msg.guild.member(user).displayName
        ]),
        description: replaceArgs(
          language.commands.register.labels.removeDescription,
          [msg.guild.member(user), args[1]]
        )
      })
      return
    }
    ...
    if (returnValue) {
      messageHandler.sendRichTextDefault({
        msg: msg,
        title: replaceArgs(language.commands.register.labels.title, [
          msg.guild.member(user).displayName
        ]),
        description: replaceArgs(
          language.commands.register.labels.description,
          [msg.guild.member(user), args[1]]
        )
      })
    }
