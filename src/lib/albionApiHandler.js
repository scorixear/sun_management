...
async function clearAlbionMembers() {
  ...
  for (const guild of discordHandler.client.guilds.cache) {
    ...
    messageHandler.sendRichTextDefaultExplicit({
      guild: guild,
      channel: channel,
      title: replaceArgs(language.commands.removeMembers.labels.title, [
        removedPlayers.length
      ]),
      description: replaceArgs(
        language.commands.removeMembers.labels.description,
        [removedPlayers.join('\n')]
      )
    })
  }
...
async function removeRoles() {
  ...
  for (const guild of discordHandler.client.guilds.cache) {
    removedPlayers.push(
      await discordHandler.removeRolesFromPlayers(guild[1], guildMembers)
    )
  }
  return removedPlayers
}
